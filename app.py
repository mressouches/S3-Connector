...
Users paste their S3 credentials in the sidebar: either as a ``key=value`` block
or field-by-field. Credentials are held in st.session_state for the duration of
...
_DEFAULT_CONNECTION_BLOCK = """# Optional: leave empty for default AWS S3
endpoint_url=
access_key=
secret_key=
bucket=
...
def _parse_connection_block(text: str) -> Dict[str, str]:
    """Parse key=value lines from a pasted configuration block.

    Empty lines and lines starting with ``#`` are ignored. Each non-empty line
    must contain ``=``; the value may be empty. Keys are matched case-insensitively
    and normalized using :data:`_CONNECTION_KEY_ALIASES` (e.g. ``AWS_ACCESS_KEY_ID``).

    Args:
        text: Raw multiline string from the user.

    Returns:
        Mapping of canonical field names to stripped string values. Unknown keys
        are skipped. Missing keys are not present in the dict.
    """
    out: Dict[str, str] = {}
    for raw_line in text.splitlines():
        ...
        key_norm = key_part.strip().lower().replace("-", "_")
        canonical = _CONNECTION_KEY_ALIASES.get(key_norm)
        if canonical:
            out[canonical] = value_part.strip()
    return out
...
def _render_connection_form() -> Optional[S3Client]:
    ...
    Offers two modes: paste a ``key=value`` block (template pre-filled) or fill
    individual fields. The mode selector is outside the form so switching modes
    applies immediately. On successful connection, stores the S3Client in
    session_state. Returns None if no client exists or connection failed.
    ...
    if "s3_connection_block" not in st.session_state:
        st.session_state["s3_connection_block"] = _DEFAULT_CONNECTION_BLOCK

    # Outside the form so changing mode triggers an immediate rerun (widgets inside
    # a form only update when the form is submitted).
    mode = st.sidebar.radio(
        "Connection mode",
        ["Configuration block", "Individual fields"],
        horizontal=True,
        key="s3_connection_mode",
        help="Paste several key=value lines at once, or use separate inputs.",
    )
    ...
    with st.sidebar.form("s3_form"):
        ...
        if mode == "Configuration block":
            st.text_area(
                "Paste or edit (key=value per line)",
                height=168,
                key="s3_connection_block",
                help=(
                    "One line per setting. Lines with # are comments. Leave a value "
                    "empty if not needed (endpoint/prefix optional). Required: "
    ...
    if connect:
        if mode == "Configuration block":
            parsed = _parse_connection_block(
                str(st.session_state.get("s3_connection_block", ""))
            )
            endpoint_url = parsed.get("endpoint_url", "").strip()
            access_key = parsed.get("access_key", "").strip()
            secret_key = parsed.get("secret_key", "").strip()
            bucket = parsed.get("bucket", "").strip()
        ...
        if not access_key or not secret_key or not bucket:
            st.sidebar.error(
                "Access Key, Secret Key and Bucket are required "
                "(in the pasted block or in the individual fields)."
            )
            return None
